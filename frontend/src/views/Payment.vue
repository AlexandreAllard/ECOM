<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="p-6 bg-white rounded-lg shadow-lg max-w-md w-full">
      ...
      <input v-model="address" type="text" placeholder="Enter your address" class="p-2 border border-gray-300 rounded mb-4 w-full">
      <button @click="submitPayment" :disabled="!clientSecret" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded disabled:opacity-50 disabled:cursor-not-allowed">
        Payer maintenant
      </button>
    </div>
  </div>
  ...
  methods: {
    ...
    async submitPayment() {
      const { error, paymentIntent } = await this.stripe.confirmPayment({
        ...
        confirmParams: {
          return_url: `${window.location.origin}/payment-success?paymentIntentId=${this.clientSecret}&address=${encodeURIComponent(this.address)}`,
        },
      });
      if (error) {
        ...
        window.location.href = `${window.location.origin}/payment-success?paymentIntentId=${paymentIntent.id}&address=${encodeURIComponent(this.address)}`;
      }
    }
  },
  mounted() {
