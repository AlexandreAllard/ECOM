...
export default {
  ...
  methods: {
    async fetchProducts() {
      ...
      try {
        const [productsResponse, searchResponse] = await Promise.all([
          axios.get('http://localhost:3000/productss'),
          this.searchQuery ? axios.get('http://localhost:3000/productss/search', { params: { search: this.searchQuery } }) : Promise.resolve({ data: [] })
        ]);
    ...
    async addToCart(product) {
      try {
        await axios.post(`http://localhost:3000/cartss/${product.id}`, { quantity: 1 }, { withCredentials: true });
